"""
# SLAM-GUI
# This script is used to visualize the Gaussian Splatting results in real-time.
# The GUI is implemented using Open3D's GUI module.
# Main functions:
# `__init__` and `init_widget` function initializes the GUI window and the widgets.
# `scene_update` function updates the scene with the current camera pose and the gaussians.
#       `receive_data` function receives the GaussianPacket from the main process and updates the GUI.
"""
import pathlib
import threading
import time
...
class SLAM_GUI:
    def __init__(self, params_gui=None, path=None):
        self.step = 0
    ...
    def init_widget(self):
        ...
        ## reliability option
        self.reli_chbox = gui.Checkbox("Reliability")
        self.reli_chbox.checked = False
        chbox_tile_geometry.add_child(self.reli_chbox)

        ## mutual info option
        self.mu_info_chbox = gui.Checkbox("GauSS-MI")
        self.mu_info_chbox.checked = False
        chbox_tile_geometry.add_child(self.mu_info_chbox)
        ...
        self.output_info2 = gui.Label("SH degree: ")
        tab_info.add_child(self.output_info2)
        self.output_info3 = gui.Label("Current View MI: ")
        tab_info.add_child(self.output_info3)
        self.currentMI = 0.0
    ...
    def add_camera(self, camera, name, color=[0, 1, 0], gt=False, size=0.001):
        W2C = (
            getWorld2View2(camera.R_gt, camera.T_gt) if gt
    ...
    def _on_screenshot_btn(self):
        ...
        save_dir = self.save_path / "screenshots"
        ...
        cv2.imwrite(f"{filename}-cam-{dt}.png", img)
        
        img = np.asarray(self.render_img)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        cv2.imwrite(f"{filename}-{dt}.png", img)
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.has_gaussians:
            ...
            self.output_info2.text = "SH degree: {}".format(
                self.gaussian_cur.active_sh_degree
            )
            self.init = True
    ...
    def get_current_cam(self):
        ...
        current_cam.set_RT(T[0:3, 0:3], T[0:3, 3])
        return current_cam
    ...
    def rasterise(self, current_cam):
        rendering_data = render(
            ...
            raster_reli = self.reli_chbox.checked,
        )
    ...
    def render_o3d_image(self, results, current_cam):
        ...
        self.currentMI = int(np.sum(mutual_info)) / 1000.0
        ...
        elif self.reli_chbox.checked:
            mutual_info = results["mutual_info"]
            mutual_info = mutual_info[0, :, :].detach().cpu().numpy()
            mutual_info = imgviz.depth2rgb(
                mutual_info, min_value=0.2, max_value=2.0, colormap="magma"
            )
            mutual_info = torch.from_numpy(mutual_info)
            mutual_info = torch.permute(mutual_info, (2, 0, 1)).float()
            mutual_info = (mutual_info).byte().permute(1, 2, 0).contiguous().cpu().numpy()
            render_img = o3d.geometry.Image(mutual_info)
            self.currentMI = 0.0
        
        elif self.mu_info_chbox.checked:
            mutual_info = results["mutual_info"]
            mutual_info = mutual_info[0, :, :].detach().cpu().numpy()
            mutual_info = imgviz.depth2rgb(
                mutual_info, min_value=0.0, max_value=1.0, colormap="pink"
            )
            mutual_info = torch.from_numpy(mutual_info)
            mutual_info = torch.permute(mutual_info, (2, 0, 1)).float()
            mutual_info = (mutual_info).byte().permute(1, 2, 0).contiguous().cpu().numpy()
    ...
    def render_gui(self):
        ...
        self.output_info3.text = "Current View MI: {} k".format(
            self.currentMI
        )
...
def run(params_gui=None, path=None):
    app = o3d.visualization.gui.Application.instance
    app.initialize()
    win = SLAM_GUI(params_gui, path)
    app.run()
